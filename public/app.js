...
function getDb(){
  if(!dbPromise){
    dbPromise = (async () => {
      try{
        if(window.claude && window.claude.use) return await window.claude.use("db");
...
/* ---------------------------------------------------------------------
   Load custom exercises (runs once at startup, independent of which tab
   is active) so they're merged into DATA everywhere from the start.
--------------------------------------------------------------------- */
