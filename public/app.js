...
function buildAutoBuildForm(){
  ...
  wrap.innerHTML = `
    ...
    <div style="font-size:13px; color:var(--ink-dim); margin:-6px 0 18px; max-width:640px;">
      Describe the client and Claude will draft a starting program using exercises from the movement library. The result opens in the normal editor below so you can adjust sets, reps, load and notes exactly as you would any other program.
    </div>
    <div class="abform-row">
...
function buildDailyAgenda(client, myPrograms){
  const box = document.createElement("div");
  box.className = "cmsection";
  const h = document.createElement("h3");
  h.textContent = "Weekly Agenda";
  box.appendChild(h);
  ...
  currentWeekDates().forEach(wd => {
    ...
    if(scheduled){
      ...
      progWrap.innerHTML = `<div class="agendaprogramlabel">🏋️ ${esc(scheduled.program.name || "Program")} — ${esc(scheduled.day.label || "Day")}</div>`;
      (scheduled.day.exercises || []).forEach(ex => progWrap.appendChild(buildClientExRow(ex, scheduled.program, weeks)));
...
function renderClientWeeklyGoals(client, myPrograms){
  ...
  box.className = "cmsection";
  const h = document.createElement("h3");
  h.textContent = "Tasks for the Big Picture";
  box.appendChild(h);
...
function buildClientStatsView(client){
  ...
  // weight instead of just %1RM).
  const box = document.createElement("div");
  box.className = "cmsection";
  const h = document.createElement("h3");
  h.textContent = "My Lifting Stats";
  box.appendChild(h);
  box.appendChild(buildStatsBox(null, client, renderClientModeView));
  return box;
}

function buildClientInfoSection(client){
  // The body-metrics/nutrition-preference half of a client's profile,
  // editable from their own portal view -- the same fields and BMR/macro
  // calculation the coach sees and can edit on the Clients tab, just
  // scoped down to "my own" and without the coach-only bits (notes,
  // access code, linked programs/cases/plans).
  const sec = document.createElement("div");
  sec.className = "cmsection";
  sec.innerHTML = "<h3>My Info</h3>";
...
function buildClientProgramCard(program, client){
  ...
  (program.days || []).forEach(day => {
    ...
    dayEl.className = "cmday";
    const scheduledKey = (client.trainingSchedule || {})[day.id] || "";
    dayEl.innerHTML = `
      <div class="cmdayhead">
        <h5>${esc(day.label||"Day")}</h5>
        <label class="cmdayschedule">Do this on:
    ...
    `;
    (day.exercises || []).forEach(ex => dayEl.appendChild(buildClientExRow(ex, program, weeks)));
    dayEl.querySelector("select").addEventListener("change", e => {
...
function renderClientModeView(){
  ...
  goalsSec.className = "cmsection";
  goalsSec.innerHTML = "<h3>Big Picture Goals</h3>";
  const goalsHint = document.createElement("div");
  goalsHint.className = "statshint";
  goalsHint.textContent = "Your coach can see this too.";
  goalsSec.appendChild(goalsHint);
  ...
  goalsSec.appendChild(goalsBox);
  host.appendChild(goalsSec);

  const myPrograms = programsCache.filter(p => p.clientId === client.id);

  host.appendChild(renderClientWeeklyGoals(client, myPrograms));

  host.appendChild(buildClientInfoSection(client));

  host.appendChild(buildDailyAgenda(client, myPrograms));

  const progSec = document.createElement("div");
  progSec.className = "cmsection";
  progSec.innerHTML = "<h3>My Program</h3>";
  if(!myPrograms.length){
    const p = document.createElement("div");
    p.className = "cmempty";
    p.textContent = "No program has been assigned yet — check back soon.";
    progSec.appendChild(p);
  } else {
    myPrograms.forEach(p => progSec.appendChild(buildClientProgramCard(p, client)));
  }
  host.appendChild(progSec);

  const myCases = casesCache.filter(c => c.clientId === client.id);
  if(myCases.length){
    const rehabSec = document.createElement("div");
    rehabSec.className = "cmsection";
    rehabSec.innerHTML = "<h3>My Rehab Plan</h3>";
    myCases.forEach(c => rehabSec.appendChild(buildClientCaseCard(c)));
    host.appendChild(rehabSec);
  }

  const myNutrition = nutritionCache.filter(n => n.clientId === client.id);
  if(myNutrition.length){
    const nutSec = document.createElement("div");
    nutSec.className = "cmsection";
    nutSec.innerHTML = "<h3>My Nutrition</h3>";
    myNutrition.forEach(n => nutSec.appendChild(buildClientNutritionCard(n)));
    host.appendChild(nutSec);
  }

  host.appendChild(buildClientStatsView(client));
