...
function render(){
  ...
  // A handful of exercises are deliberately listed more than once in the
  // underlying data under different Muscle Group / Sub-Region combos (e.g.
  // "Romanian Deadlift" is tagged both Glutes-›Glute Max and
  // Hamstrings-›Hamstrings) so it's still findable no matter which
  // muscle a coach filters by first -- that's intentional, not a data
  // error, and deleting either copy would make it disappear from one of
  // those two facet paths. What WAS a real bug: browsing without narrowing
  // all the way down to a single Sub-Region (a free-text search, or a
  // Muscle Group filter with no Sub-Region chosen yet) could show that
  // same exercise as two or three separate, identical-looking cards in the
  // results grid. mergeDisplayRows collapses those into one card per
  // exercise name, folding every context it's tagged under into that
  // card's tag row/movers instead, so nothing is removed from the
  // underlying library (every Muscle Group / Sub-Region combo is still
  // fully searchable) but nothing ever appears twice on screen either.
  const displayGroups = mergeDisplayRows(results);

  statusEl.textContent = displayGroups.length + (displayGroups.length === 1 ? " exercise matches" : " exercises match");
  ...
  displayGroups.forEach(dg => {
    const r = dg.rows[0]; // representative row -- demo link, region accent, and queuing all key off this one
    ...
    card.innerHTML = `
      ...
      <div class="tagrow">
        ...
        ${dg.groupSubTags.map(t => `<span class="tag">${esc(t)}</span>`).join("")}
        ${dg.planes.map(p => `<span class="tag">${esc(p)}</span>`).join("")}
        ${dg.patterns.map(p => `<span class="tag">${esc(p)}</span>`).join("")}
      </div>
      <div class="detail"><b>Joint action</b>${esc(dg.joint)}</div>
      <div class="detail"><b>Primary mover(s)</b>${esc(dg.primary)}</div>
      <div class="detail"><b>Secondary mover(s)</b>${esc(dg.secondary)}</div>
...
// Groups a filtered result set by exercise name, so the same movement
// (deliberately tagged under more than one Muscle Group / Sub-Region --
// see the comment above its call site) shows as exactly one merged card
// instead of one card per tag combo. Each returned group carries its full
// list of underlying `rows` (sorted, representative first) plus the
// pre-combined display strings for the card body.
function mergeDisplayRows(results){
  const byName = new Map();
  results.forEach(r => {
    if(!byName.has(r.exercise)) byName.set(r.exercise, []);
    byName.get(r.exercise).push(r);
  });
  ...
  const groups = Array.from(byName.entries()).map(([exercise, rows]) => {
    rows.sort((a,b) => (a.group+a.sub).localeCompare(b.group+b.sub));
    const groupSubTags = [];
    rows.forEach(r => {
      const tag = `${r.group} — ${r.sub}`;
      if(!groupSubTags.includes(tag)) groupSubTags.push(tag);
    });
    const planes = [];
    rows.forEach(r => { if(!planes.includes(r.plane)) planes.push(r.plane); });
    const patterns = [];
    rows.forEach(r => { if(!patterns.includes(r.pattern)) patterns.push(r.pattern); });
    const joints = [];
    rows.forEach(r => { if(!joints.includes(r.joint)) joints.push(r.joint); });
    return {
      exercise, rows, groupSubTags, planes, patterns,
      joint: joints.join("; "),
      primary: uniqueJoin(rows.map(r => r.primary)),
      secondary: uniqueJoin(rows.map(r => r.secondary)),
    };
  });
...
function buildAnatomyMuscleCard(key){
  ...
  card.innerHTML = html;
  const diagramWrap = buildMuscleDiagramWrap(key);
  if(diagramWrap) card.insertBefore(diagramWrap, card.firstChild);
  return card;
}
